...
logFolder=/root/llm-project/NFGen+KAN/Results/${protocol}/${network}/secure_approx_logs/
clogFolder=/root/llm-project/NFGen+KAN/Results/${protocol}/${network}/secure_approx_logs/Compile/
clogFile=${clogFolder}approx_compile_log-${N}
logFile=${logFolder}approx_execution_log-${N}
sourceFile=multivariate_secure_approx_performance
...
for func in "${target_funcs[@]}"; do
    ...
    for i in $(seq 1 ${parties_num}); do
        ...
        if [ ${player_id} -eq 0 ]; then
            ssh ${host} "cd ${MP_SPDZ_DIR} && ./${protocol} -b 1000 -p ${player_id} --ip-file-name ${MP_SPDZ_DIR}Hosts ${sourceFile}-${func}-${N}-${nth} -v" >> ${logFile}_${func}.log 2>&1 &
        else
            ssh ${host} "cd ${MP_SPDZ_DIR} && ./${protocol} -b 1000 -p ${player_id} --ip-file-name ${MP_SPDZ_DIR}Hosts ${sourceFile}-${func}-${N}-${nth} -v" &
        fi
    done
